...
主题默认跟随操作系统，手动选择日间/夜间后保存在本地浏览器；选择“跟随系统”即可恢复自动模式。没有第三方追踪脚本、广告脚本或必须同意才能阅读的弹窗。

## 隐私友好的访问统计

站点不设置或读取 Cookie。Pages Function 只记录页面浏览、文章入口点击、正文阅读至 50% 和 90% 四类事件；同一公网 IP 对同一页面的同一事件，在上海自然日内只记一次。同一 IP 从不同入口反复点击同一文章，也只记一次。服务端用私密盐值生成每日 HMAC 摘要，不保存原始 IP、User-Agent、搜索词、来源页面或连续滚动轨迹。启用 Do Not Track 或 Global Privacy Control 的浏览器不发送事件。

共享同一公网 IP 的读者会被合并，这是按 IP 去重的固有限制。统计数据存放在 Cloudflare D1 的 `analytics_events` 表中，可按 `day`、`event` 和 `path` 聚合查询。
